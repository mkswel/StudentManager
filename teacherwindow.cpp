...
void TeacherWindow::init_My() {
    ...
    qDebug()<<"所带班级"<<query.value(0).toString()<<query.value(1).toString();
    while (query.next()) {
...
void TeacherWindow::on_Cno_currentTextChanged(const QString &arg1)
{
    ...
    QSqlQuery query(db);
    query.exec("select distinct student.sno, sname, class.major, cname, score, course.grade, course.cno, student.grade "
               "from student, sc, class, course "
               "where student.sno = sc.sno and "
               "course.cno = sc.cno and "
               "student.classno like '" + classno + "' and "
